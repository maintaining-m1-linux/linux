...
static void ane_detach_genpd(struct ane_device *ane)
{
	for (int i = 4; i >= 0; i--) {
		if (ane->pm_links[i]) {
			device_link_del(ane->pm_links[i]);
			ane->pm_links[i] = NULL;
		}
		if (ane->pm_domains[i]) {
			dev_pm_domain_detach(ane->pm_domains[i], true);
			ane->pm_domains[i] = NULL;
		}
	}
}
...
static int ane_attach_genpd(struct ane_device *ane)
{
	...
	for (i = 0; i < 5; i++) {
		ane->pm_domains[i] = dev_pm_domain_attach_by_id(dev, i);
		if (IS_ERR(ane->pm_domains[i])) {
			int err = PTR_ERR(ane->pm_domains[i]);
		...
		ane->pm_links[i] =
			device_link_add(dev, ane->pm_domains[i],
					DL_FLAG_STATELESS | DL_FLAG_PM_RUNTIME |
						DL_FLAG_RPM_ACTIVE);
		if (!ane->pm_links[i]) {
			dev_err(dev, "failed to add device link for pm domain %d\n", i);
			if (ane->pm_domains[i]) {
				dev_pm_domain_detach(ane->pm_domains[i], true);
				ane->pm_domains[i] = NULL;
			}
...
static int ane_platform_probe(struct platform_device *pdev)
{
	...
	/*
	 * Enable Runtime PM prior to attaching power domains so that
	 * device_link_add with DL_FLAG_RPM_ACTIVE can succeed.
	 */
	...
	if (err < 0) {
		...
		goto disable_pm_only;
	}

	ane->irq = platform_get_irq(pdev, 0);
	if (ane->irq < 0) {
		err = ane->irq;
		goto disable_pm_only;
	}

	ane->dart_irq = platform_get_irq(pdev, 1);
	if (ane->dart_irq < 0) {
		err = ane->dart_irq;
		goto disable_pm_only;
	}
